...
public:
    parallelogram(point a, int width, int height)
        : rectangle(a, point(a.x + width, a.y + height)) {}

    void draw() {
        int width = ne.x - sw.x;
        int height = ne.y - sw.y;

        for (int y = 0; y <= height; ++y) {
            for (int x = 0; x <= width; ++x) {
                if (y <= height * (1.0 - x / double(width))) {
                    put_point(sw.x + x, sw.y + y);
                }
            }
        }
    }
